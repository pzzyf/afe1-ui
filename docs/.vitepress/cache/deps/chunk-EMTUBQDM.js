// ../node_modules/.pnpm/@vue+shared@3.3.10/node_modules/@vue/shared/dist/shared.esm-bundler.js
...
// ../node_modules/.pnpm/@vue+reactivity@3.3.10/node_modules/@vue/reactivity/dist/reactivity.esm-bundler.js
...
const BaseReactiveHandler = class {
...
const MutableReactiveHandler = class extends BaseReactiveHandler {
...
const ReadonlyReactiveHandler = class extends BaseReactiveHandler {
  constructor(shallow = false) {
    super(true, shallow)
  }
  set(target, key) {
...
// ../node_modules/.pnpm/@vue+runtime-core@3.3.10/node_modules/@vue/runtime-core/dist/runtime-core.esm-bundler.js
const stack = []
...
function callWithAsyncErrorHandling(fn, instance, type, args) {
  ...
  for (const element of fn) {
    values.push(callWithAsyncErrorHandling(element, instance, type, args))
  }
  return values
}
  ...
  if (instance) {
    ...
    while (cur) {
      ...
      if (errorCapturedHooks) {
        for (const errorCapturedHook of errorCapturedHooks) {
          if (errorCapturedHook(err, exposedInstance, errorInfo) === false) {
            return
          }
        }
      }
...
var getId = (job) => (job.id == null ? Number.POSITIVE_INFINITY : job.id)
...
function normalizeSuspenseSlot(s) {
  ...
  if (isArray(s)) {
    ...
    if (!singleChild && s.some((child) => child !== NULL_DYNAMIC_COMPONENT)) {
...
function withDirectives(vnode, directives) {
  ...
  for (let [dir, value, arg, modifiers = EMPTY_OBJ] of directives) {
    if (dir) {
      if (isFunction(dir)) {
        dir = {
          mounted: dir,
...
function normalizePropsOptions(comp, appContext, asMixin = false) {
  ...
  if (isArray(raw)) {
    for (const element of raw) {
      if (!isString(element)) {
        warn2(`props must be strings when using array syntax.`, element)
      }
      const normalizedKey = camelize(element)
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const setScopeId = (el, vnode, scopeId, slotScopeIds, parentComponent) => {
    ...
    if (slotScopeIds) {
      for (const slotScopeId of slotScopeIds) {
        hostSetScopeId(el, slotScopeId)
      }
    }
    if (parentComponent) {
  ...
  const move = (vnode, container, anchor, moveType, parentSuspense = null) => {
    ...
    if (type === Fragment) {
      hostInsert(el, container, anchor)
      for (const child of children) {
        move(child, container, anchor, moveType)
      }
...
function moveTeleport(
  ...
  if (!isReorder || isTeleportDisabled(props)) {
    if (shapeFlag & 16) {
      for (const child of children) {
        move(child, container, parentAnchor, 2)
      }
    }
  }
  if (isReorder) {
...
function updateCssVars(vnode) {
  ...
  if (ctx && ctx.ut) {
    ...
    while (node && node !== vnode.targetAnchor) {
      if (node.nodeType === 1) node.dataset.vOwner = ctx.uid
  ...
  if (true) {
    ...
    if (Component.components) {
      ...
      for (const name_ of names) {
        validateComponentName(name_, instance.appContext.config)
      }
    }
    if (Component.directives) {
      const names = Object.keys(Component.directives)
      for (const name_ of names) {
        validateDirectiveName(name_)
      }
    }
...
// ../node_modules/.pnpm/@vue+runtime-dom@3.3.10/node_modules/@vue/runtime-dom/dist/runtime-dom.esm-bundler.js
...
const enterCbKey2 = Symbol('_enterCb')
...
// ../node_modules/.pnpm/vue@3.3.10_typescript@5.3.3/node_modules/vue/dist/vue.runtime.esm-bundler.js
function initDev() {
  {
    initCustomFormatter()
  }
...
@vue/runtime-dom/dist/runtime-dom.esm-bundler.js:
  (*! #__NO_SIDE_EFFECTS__ *)
*/
//# sourceMappingURL=chunk-EMTUBQDM.js.map
