...
const DOMIterator = class _DOMIterator {
  /**
  ...
  getContexts() {
    ...
    ctx.forEach((ctx2) => {
      const isDescendant = filteredCtx.some((contexts) => {
        return contexts.contains(ctx2)
      })
      if (!filteredCtx.includes(ctx2) && !isDescendant) {
      ...
      if (syn.hasOwnProperty(index)) {
        ...
        if (k1 !== '' && k2 !== '') {
          str = str.replace(
            ...
            `${joinerPlaceholder}(${this.processSynomyms(
              k1
  ...
  getSeparatedKeywords(sv) {
    ...
    sv.forEach((kw) => {
      if (!this.opt.separateWordSearch) {
        if (kw.trim() && !stack.includes(kw)) {
          stack.push(kw)
        }
      } else {
        kw.split(' ').forEach((kwSplitted) => {
          if (kwSplitted.trim() && !stack.includes(kwSplitted)) {
    ...
    dict.nodes.every((n, i) => {
      ...
      if (typeof sibl === 'undefined' || sibl.start > start) {
        ...
        const s = start - n.start,
          ...
          startStr = dict.value.slice(0, Math.max(0, n.start)),
          endStr = dict.value.slice(e + n.start)
